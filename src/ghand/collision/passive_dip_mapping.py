...
def _normalize_finger_identifier(finger: str) -> str:
    ...
    normalized_finger = finger.lower()
...
_FINGER_RANGE_MAP = {
    'index': _STD_PIP_RANGE,
    'middle': _STD_PIP_RANGE,
    'ring': _STD_PIP_RANGE,
    'little': _LITTLE_PIP_RANGE,
    'thumb': _THUMB_PIP_RANGE,
}
...
def get_supported_pip_range(finger: str) -> tuple[float, float]:
    ...
    Args:
        finger: Finger identifier (little/ring/middle/index/thumb).
...
def solve_thumb_dip_from_pip(pip_rad: float) -> float:
    ...
    Note:
        In the A7 collision-detection convention, Thumb_IP is driven by
        Thumb_MCP.
...
def solve_finger_dip_from_pip(pip_rad: float, finger: str) -> float:
    ...
    """
    normalized_finger = _normalize_finger_identifier(finger)
    if normalized_finger == 'little':
        return solve_little_finger_dip_from_pip(pip_rad)
    if normalized_finger == 'thumb':
        return solve_thumb_dip_from_pip(pip_rad)
    if normalized_finger in {'index', 'middle', 'ring'}:
