...
class DemoScene:
    ...
    hold_command_mode: HoldCommandMode = HoldCommandMode.POSITION
...
DEMO_SCENES: dict[str, DemoScene] = {
    "paper_cup":
    DemoScene(default_object="paper_cup",
              pre_grasp_preset="paper_cup_grasp",
              hold_command_mode=HoldCommandMode.POSITION),
    "balloon":
    DemoScene(default_object="balloon",
              pre_grasp_preset="balloon_pinch",
              hold_command_mode=HoldCommandMode.POSITION),
    "glass_cup":
    DemoScene(default_object="glass",
              pre_grasp_preset="three_finger_grasp",
              hold_command_mode=HoldCommandMode.STIFF_POSITION),
    "plastic_cup":
    DemoScene(default_object="plastic_cup",
              pre_grasp_preset="paper_cup_grasp",
              hold_command_mode=HoldCommandMode.POSITION),
    "mineral_water_bottle_500ml":
    DemoScene(
    ...
    "plastic_object":
    DemoScene(default_object="plastic",
              pre_grasp_preset="two_finger_pinch",
              hold_command_mode=HoldCommandMode.POSITION),
    "orange":
    DemoScene(default_object="fruit",
              pre_grasp_preset="four_finger_grasp",
              hold_command_mode=HoldCommandMode.POSITION),
    "cylinder_piece":
    DemoScene(default_object="cylinder_piece",
              pre_grasp_preset="cylinder_piece_grasp",
              hold_command_mode=HoldCommandMode.STIFF_POSITION),
}
...
def build_demo_runtime_config(
    ...
    if grasp_object not in DEMO_SCENES:
        ...
        raise ValueError(f'Unsupported GRASP_OBJECT="{grasp_object}". '
