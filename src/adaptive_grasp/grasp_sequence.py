...
class ClosingSensorFrame:
    ...
    joint_feedback: list[JointCommand]
    total_fz: float
...
class PhaseController:
    ...
    def _read_closing_sensor_frame(self) -> Optional[ClosingSensorFrame]:
        ...
        if tactile_data is None or joint_feedback is None:
            _logger.error(
                ...
                "tactile data" if tactile_data is None else "JointCommand feedback",
            )
            return None
    ...
    def _is_empty_grasp(self, joint_feedback: list[JointCommand]) -> bool:
    ...
    def _try_confirm_stall_contact(
        ...
        _logger.debug(
            "CLOSING: JointCommand stall detected (%d/%d)",
            stall_counter,
