    ...
    *,
    ...
    return ContactSnapshot(
        ...
        if finger_fz is not None
            ...
            TactileSensorId.FF: 0.15,
            TactileSensorId.MF: 0.10,
        },
        total_fz=total_fz,
        torque=5,
...
def test_force_reference_initializes_contact_ratios_from_snapshot():
    ...
    assert planner.contact_ratios[TactileSensorId.THUMB] > planner.contact_ratios[TactileSensorId.FF]
    assert planner.contact_ratios[TactileSensorId.FF] > planner.contact_ratios[TactileSensorId.MF]
    assert planner.contact_ratios[TactileSensorId.MF] >= 0.15 - 1e-6
...
def test_force_reference_uses_uniform_ratios_when_snapshot_force_is_zero():
    ...
    assert planner.contact_ratios == {
        ...
        TactileSensorId.FF: pytest.approx(1 / 3),
        TactileSensorId.MF: pytest.approx(1 / 3),
    }
...
def test_force_reference_compute_returns_per_finger_refs():
    ...
    assert decision.force_refs[TactileSensorId.FF] == pytest.approx(0.15)
    assert decision.force_refs[TactileSensorId.MF] == pytest.approx(0.10)
...
def test_force_reference_increases_only_high_risk_finger_ref():
    ...
    assert decision.force_refs[TactileSensorId.FF] == pytest.approx(
        initial[TactileSensorId.FF]
    )
    assert decision.force_refs[TactileSensorId.MF] == pytest.approx(
        initial[TactileSensorId.MF]
    )
