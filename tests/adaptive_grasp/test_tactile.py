...
from adaptive_grasp.tactility import (
...
class GHandTactileInfo:
    def __init__(self, fx, fy, fz, distributed=None):
        self.resultant_force = [fx, fy, fz]
        self.distributed_force = distributed if distributed is not None else []


def test_tactile_layout_points_match_sensor_geometry():
    thumb_points = get_tactile_layout_points(TactileSensorId.THUMB)
    forefinger_points = get_tactile_layout_points(TactileSensorId.FF)
...
def test_tactile_analysis_variance_and_slip_risk():
    ...
    for i in range(cfg.tactile_slip_window_size):
        data = {
            ...
            TactileSensorId.FF: FakeTactileInfo(0.0, 0.0, 1.0),
        }
...
def test_tactile_analysis_accepts_ghand_tactile_fields():
    ...
    analyzer.update({TactileSensorId.THUMB: GHandTactileInfo(0.0, 0.0, 1.0, stable)})
    result = analyzer.update(
        {TactileSensorId.THUMB: GHandTactileInfo(0.3, 0.4, -2.0, stable)}
    )
...
def test_direction_distance_with_multiple_fingers():
    ...
    analyzer.update({
        ...
        TactileSensorId.FF: FakeTactileInfo(2.0, 0.0, 1.0, distributed=f2),
    })
...
def test_friction_utilization_multiple_fingers_takes_max():
    ...
    result = analyzer.update({
        ...
        TactileSensorId.FF: FakeTactileInfo(0.4, 0.0, 1.0),  # mu_eff=0.4 => r=0.8
    })
...
def test_per_finger_analysis_exposes_individual_slip_risk():
    ...
    forefinger_risk = result.per_finger[TactileSensorId.FF].slip_risk
