...
force_ref_slip_gain_n_per_s: 滑动风险响应增益，配置在 AdaptiveGraspConfig；它不是倒水速度，而是当触觉检测到滑动趋势时，允许 `F_ref_total` 随 `slip_risk` 增加的最大响应强度。
force_ref_max_rise_step_n: 单个控制周期允许的最大增力步长，配置在 AdaptiveGraspConfig，用作安全限幅，避免触觉噪声导致力目标突跳。
force_ref_confirmed_boost_n: 确认滑动时的一次性目标力增量，配置在 AdaptiveGraspConfig，建议小于等于 0.5 N；默认仍使用更保守的小值，避免阶跃压坏杯壁。
```
...
不要把 force_ref_margin_n、force_ref_slip_gain_n_per_s、force_ref_max_rise_step_n、force_ref_confirmed_boost_n、torque_hold_K_p/K_i/K_d 这类控制器调参项放进 ObjectProfile。
...
```python
force_ref_margin_n
force_ref_slip_warning_threshold
force_ref_stable_threshold
force_ref_slip_gain_n_per_s
force_ref_max_rise_step_n
force_ref_confirmed_boost_n
force_ref_decay_rate_n_per_s
force_ref_stable_decay_delay_s
force_ref_min_contact_ratio
torque_hold_K_p
torque_hold_K_i
torque_hold_K_d
torque_hold_I_min
...
计算比例时保留接触快照中的逐指法向力原始读数，不做低力过滤；比例下限由 `force_ref_min_contact_ratio` 负责兜底。
...
def test_torque_hold_planner_initializes_contact_ratios_from_snapshot():
    ...
    cfg = AdaptiveGraspConfig(
        ...
        force_ref_min_contact_ratio=0.15,
    )
...
def test_torque_hold_planner_does_not_repeat_confirmed_boost():
    ...
    assert planner.F_ref_total < boosted + planner.config.force_ref_confirmed_boost_n
```
...
def _update_total_force_ref(self, analysis: TactileAnalysis, dt: float) -> None:
    warning_threshold = self.config.force_ref_slip_warning_threshold
    stable_threshold = self.config.force_ref_stable_threshold
    slip_gain = self.config.force_ref_slip_gain_n_per_s
    max_rise_step = self.config.force_ref_max_rise_step_n
    boost = self.config.force_ref_confirmed_boost_n
    decay_rate = self.config.force_ref_decay_rate_n_per_s
    decay_delay = self.config.force_ref_stable_decay_delay_s
...
2. AdaptiveGraspConfig.force_ref_margin_n
3. AdaptiveGraspConfig.force_ref_confirmed_boost_n
4. AdaptiveGraspConfig.force_ref_slip_gain_n_per_s
5. AdaptiveGraspConfig.force_ref_max_rise_step_n
