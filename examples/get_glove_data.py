...
def process_glove_data(data):
    ...
    if len(data_items) > 1:
        ...
        # 根据描述，我们需要每根手指的mcp和pip传感器数据（每6个为一组，取每组的第4和第5个数据）
        # 右手数据索引（基于每组6个数据）：
        right_hand_data = []
        if len(numeric_data) >= 96:  # 确保有足够的数据
            # 右手拇指 (组 1, 2)
            right_hand_data.extend([
                numeric_data[10], numeric_data[9],  # mcp bend, sway (组1: 索引6-11)
                numeric_data[16], numeric_data[15]   # pip bend, sway (组2: 索引12-17)
            ])
            # 右手食指 (组 4, 5)
            right_hand_data.extend([
        ...
        if len(numeric_data) >= 192:  # 确保有足够的数据
            ...
            left_hand_data.extend([
                numeric_data[106], numeric_data[105],  # mcp bend, sway (组17: 索引102-107)
                numeric_data[112], numeric_data[111]   # pip bend, sway (组18: 索引108-113)
            ])
            # 左手食指 (组 20, 21)
            left_hand_data.extend([
        ...
        if current_time - last_process_time >= process_interval:
            ...
            if left_hand:
                ...
                joints.append(Joint(id=JointId.THUMB_PIP, 
                                   angle=clip_angle_radians(left_hand.thumb.pip_bend, 0, 75), speed=1000, torque=90))
                joints.append(Joint(id=JointId.THUMB_MCP, 
                                   angle=clip_angle_radians(left_hand.thumb.mcp_bend, 0, 55), speed=1000, torque=90))
                joints.append(Joint(id=JointId.THUMB_SWING, 
                                   angle=clip_angle_radians(left_hand.thumb.mcp_sway + left_hand.thumb.pip_sway, 0, 90), speed=50, torque=90))
                joints.append(Joint(id=JointId.THUMB_ROTATION, 
                                   angle=clip_angle_radians(0, 0, 90), speed=50, torque=90))
