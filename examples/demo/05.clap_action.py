...
def hand_clap(hand):
    joints = [JointCommand(id=joint_id, angle=angle, speed=100, torque=100) for joint_id, angle in clap_hand.items()]
    ...
    try:
        ...
        while True:
            ...
            if not hand_clap(hand):
                print(f"Cycle {gesture_cycle}: Clap action failed")
                break
            time.sleep(5)
