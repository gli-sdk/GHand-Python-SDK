...
def hand_pull(hand):
    joints = [JointCommand(id=joint_id, angle=angle, speed=100, torque=100) for joint_id, angle in pull_tightly.items()]
    ...
    try:
        ...
        while True:
            ...
            if not hand_pull(hand):
                print(f"Cycle {gesture_cycle}: Pull action failed")
                break
            time.sleep(5)
