...
GHand Python SDK 是为枭尧灵巧手设计的官方开发工具包。它提供了一套完整的 Python API，允许开发者轻松地与灵巧手进行交互，实现对关节、传感器以及其他核心功能的精确控制和数据获取。
...
正确配置 Python 的环境后，请确保您的系统已安装 C/C++ 编译工具和 Npcap 库。
...
`examples/` 目录下提供了多个 Python 脚本，演示了如何使用 SDK 的各项功能。您可以通过运行这些脚本来快速了解 SDK 的用法。
...
```bash
# 进入 SDK 目录，如果 SDK 不在 ~/ghand-sdk，请使用实际路径。
cd ~/ghand-sdk
```
...
```bash
# 在虚拟环境中使用 sudo 运行
sudo venv/bin/python3 examples/tutorial/01.get_basic_info.py
```
...
```python
from ghand import configure_logging, GHand
import logging
...
```python
from ghand import configure_logging_file

# 将日志写入文件（DEBUG 级别）
configure_logging_file("ghand.log", level=logging.DEBUG)
```
...
```python
from ghand import configure_logging, configure_logging_file
import logging
...
```python
import logging
from ghand import get_logger

# 只显示特定模块的日志
ghand_logger = get_logger("ghand")
ghand_logger.setLevel(logging.DEBUG)
```
...
SDK 提供以下异常类（继承自基类 `GHandError`）：

| 异常类 | 使用场景 |
|--------|----------|
| `CommunicationError` | 设备断连或通信/数据错误 |
| `HandStateError` | 设备或关节异常状态（可通过 `clear_fault()` 清除） |

### 基本用法

```python
from ghand import GHand, CommunicationError, HandStateError

hand = GHand()

try:
    hand.open()
    info = hand.get_hand_info()  # 可能抛出 HandStateError
    joints = hand.get_joints()   # 可能抛出 HandStateError
...
```python
from ghand import HandStateError, ErrorCode, State

except HandStateError as e:
    # 异常信息已包含在异常消息中
    print(f"异常详情: {e}")
    # 如需进一步处理，可调用 get_hand_info() 或 get_joints() 获取当前状态
```
...
```python
from ghand import GHand, CommType, JointCommand, JointId
from ghand.converter import nparray_to_joints
import math
...
# 准备目标关节
target_joints = [
    JointCommand(id=JointId.THUMB_PIP, angle=math.radians(30), speed=100, torque=100),
    JointCommand(id=JointId.FF_PIP, angle=math.radians(30), speed=100, torque=100),
    # ...
]
...
if result.has_collision:
    print(f"检测到碰撞: {result.collision_pairs}")
    safe_joints = nparray_to_joints(result.safe_angles, speed=100, torque=100)
...
- **`check_collision(joints)`** - 检查目标关节姿态是否会发生碰撞
  - 仅进行检测，不会执行任何关节运动
  - 返回 `CollisionCheckResult`，包含以下字段：
    - `has_collision` (`bool`): 是否检测到碰撞
    - `safe_angles` (`numpy.ndarray`): 安全角度数组（未碰撞时等于目标角度）
    - `collision_pairs` (`list[str] | None`): 发生碰撞的关节/连杆名称对

### 示例程序

查看 `examples/extension/` 目录中的示例：
- `01.collision_detection.py` - 碰撞检测演示
