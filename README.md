...
SDK 提供了可选的碰撞检测功能，可以自动检测手指间和手指与手掌之间的碰撞，避免硬件损坏。

要启用碰撞检测功能，请安装额外的依赖：

```bash
pip install -e .[collision]
```

或手动安装依赖文件：

```bash
pip install -r requirements-collision.txt
```

**注意**：碰撞检测功能需要以下依赖项：numpy, pandas, trimesh, numpy-stl, networkx, scipy（约 200MB）。如果不使用碰撞检测功能，可以跳过此步骤。
...
- `20.collision_detection_offline.py` - 离线碰撞检测演示
- `21.collision_detection_online.py` - 在线碰撞检测演示
- `22.collision_with_safe_margin.py` - 安全边距设置演示

### 注意事项

- **DIP 关节自动计算**：远端指间关节（DIP）会根据近端指间关节（PIP）自动计算，符合灵巧手的机械约束
- **性能影响**：首次启用时需要 1-2 秒加载数据，单次检测约 10-50ms
- **依赖要求**：需要安装额外的依赖项（约 200MB）
